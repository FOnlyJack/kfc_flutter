...
import 'package:kfc_flutter/bean/city_model.dart';
import 'package:kfc_flutter/view/ImagesAnimation.dart';
import 'package:kfc_flutter/view/PopRoute.dart';
import 'package:lpinyin/lpinyin.dart';

class CitySelectCustomHeaderRoute extends ProgressIndicator {
  @override
...
//              Navigator.pop(context, model);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        child: FutureBuilder(
            future: Future.delayed(Duration(seconds: 3)).then((val) {
              return DefaultAssetBundle.of(context)
                  .loadString("assets/city.json");
            }),
            builder: (context, val) {
              if (val.hasData) {
                Map countyMap = json.decode(val.data);
